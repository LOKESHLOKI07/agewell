...
interface FamilyVisitCardProps {
  visit: Visit;
  onPress?: () => void;
}

export function FamilyVisitCard({ visit, onPress }: FamilyVisitCardProps) {
  ...
  const lines = [visit.careManagerName ?? 'Care manager not assigned', visit.employeeId ? `Employee ${visit.employeeId}` : null, when, visit.notes].filter(
    ...
    <View style={styles.card}>
      ...
      <View style={styles.body}>
        <View style={styles.top}>
          <Text style={styles.title}>{visit.careManagerName ?? 'Visit'}</Text>
          <StatusBadge
            presentation={{
              label: status,
